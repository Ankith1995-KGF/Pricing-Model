...
def malaa_factor(score:int)->float: return float(np.clip(1.45 - (score-300)*(0.90/600), 0.55, 1.45))
def ltv_factor(ltv: float)->float: return float(np.clip(0.55 + 0.0075*ltv, 0.80, 1.50))
...
# ---------- Main app ----------
def main():
    st.set_page_config(page_title="rt 360 Pricing Model", page_icon="💠", layout="wide")
    st.markdown("<h1>rt 360 — Risk-Adjusted Pricing Model</h1>", unsafe_allow_html=True)
    tab1, tab2, tab3 = st.tabs(["Single Loan Pricing", "Loan Book Upload", "About"])

    with tab1:
        st.subheader("Single Loan Inputs")
        colmkt, colloan = st.columns(2)
        with colmkt:
        ...
        with colloan:
            ...
            malaa_score = int(st.number_input("Mala’a Score", value=750, step=1, format="%d"))
        ...
        if st.button("Compute Pricing"):
            rb = composite_risk(product, industry, malaa_score, ltv_pct if is_fund else 60.0, wc_omr, sales_omr, is_fund)
            pdv = pd_from_risk(rb, stage)
            lgdv = lgd_from_product_ltv(product, ltv_pct, is_fund)
            prov_pct = f2(pdv * (lgdv / 100.0))
            malaa_lbl = malaa_label(malaa_score)

            results = []
            for bucket in BUCKETS:
                risk_b = float(np.clip(rb * BUCKET_MULT[bucket], 0.4, 3.5))
                pd_pct = pd_from_risk(risk_b, stage)
                lgd_pct = lgd_from_product_ltv(product, ltv_pct if is_fund else 60.0, is_fund)
                prov_b = f2(pd_pct * (lgd_pct / 100.0))
                raw_bps = base_spread_from_risk(risk_b)
                floors = BUCKET_FLOOR_BPS[bucket] + MALAA_FLOOR_BPS[malaa_lbl] + industry_floor_addon(industry_factor[industry]) + product_floor_addon(product)
                base_bps = max(int(round(raw_bps)), floors, 125)
                band_bps = BUCKET_BAND_BPS[bucket]
                spread_min_bps = max(base_bps - band_bps, 125)
                spread_max_bps = spread_min_bps + 2*band_bps
                rate_min = clamp(oibor_pct + spread_min_bps/100.0, 5.0, 12.0)
                rate_max = clamp(oibor_pct + spread_max_bps/100.0, 5.0, 12.0)
                rep_rate = max((rate_min + rate_max)/2.0, cof_pct + prov_b + opex_pct - fees_pct + target_nim_pct)
                nim = rep_rate + fees_pct - (cof_pct + prov_b + opex_pct)
                results.append({
                    "Bucket": bucket, "Risk": f2(risk_b),
                    "PD %": f2(pd_pct), "LGD %": f2(lgd_pct), "Prov %": prov_b,
                    "Rate Min %": f2(rate_min), "Rep Rate %": f2(rep_rate), "Rate Max %": f2(rate_max),
                    "Spread Min bps": spread_min_bps, "Spread Max bps": spread_max_bps, "NIM %": f2(nim)
                })
            st.dataframe(pd.DataFrame(results).round(2), use_container_width=True)

    with tab2:
        st.subheader("Batch Loan Book Pricing")
        uploaded_file = st.file_uploader("Upload Loan Book (CSV)", type=["csv"])
        if uploaded_file:
            df = pd.read_csv(uploaded_file)
            st.write(f"Loaded {len(df)} loans.")
            batch_results = []
            for _, loan in df.iterrows():
                lp_prod = loan["product"]
                lp_ind = loan["industry"]
                is_f = lp_prod in PRODUCTS_FUND
                rb = composite_risk(lp_prod, lp_ind, loan["malaa_score"], loan.get("ltv_pct", float("nan")) if is_f else 60.0, loan.get("working_capital_omr", 0.0), loan.get("sales_omr", 0.0), is_f)
                pdv = pd_from_risk(rb, loan["stage"])
                lgdv = lgd_from_product_ltv(lp_prod, loan.get("ltv_pct", float("nan")), is_f)
                prov_pct = f2(pdv * (lgdv / 100.0))
                fees_pct = 0.4 if lp_prod in PRODUCTS_UTIL or lp_prod == "Export Finance" else 0.0
                for bucket in BUCKETS:
                    risk_b = float(np.clip(rb * BUCKET_MULT[bucket], 0.4, 3.5))
                    pd_pct = pd_from_risk(risk_b, loan["stage"])
                    lgd_pct = lgd_from_product_ltv(lp_prod, loan.get("ltv_pct", float("nan")) if is_f else 60.0, is_f)
                    prov_b = f2(pd_pct * (lgd_pct / 100.0))
                    raw_bps = base_spread_from_risk(risk_b)
                    floors = BUCKET_FLOOR_BPS[bucket] + MALAA_FLOOR_BPS[malaa_label(loan["malaa_score"])] + industry_floor_addon(industry_factor[lp_ind]) + product_floor_addon(lp_prod)
                    base_bps = max(int(round(raw_bps)), floors, 125)
                    band_bps = BUCKET_BAND_BPS[bucket]
                    spread_min_bps = max(base_bps - band_bps, 125)
                    spread_max_bps = spread_min_bps + 2*band_bps
            ...
            out = pd.DataFrame(batch_results).round(2)
            st.dataframe(out, use_container_width=True)
            st.download_button("Download Results CSV", data=out.to_csv(index=False).encode(),
                               file_name="loan_book_results.csv", mime="text/csv")

    with tab3:
        st.write("About / Methodology: This model computes credit-risk adjusted loan pricing...")
